...
在未开启该功能时，options 在 `compiler.hooks.afterEnvironment` 调用之后仍然可以对一些 options 进行有效的修改。

而在开启该功能后，options 在 `compiler.hooks.afterEnvironment` 调用之后不会再进行 options 的有效修改。该行为与 Webpack 一致，所以该配置对于使用 Rspack 开发应用的用户来说基本没有影响，而对于 Rspack 的插件或上层框架开发者需要注意。

一个常见的场景就是在[创建 Compiler 之后对 entry 进行一些修改](https://github.com/webpack/webpack-dev-server/blob/540c43852ea33f9cb18820e1cef05d5ddb86cc3e/lib/Server.js#L719-L783)：
