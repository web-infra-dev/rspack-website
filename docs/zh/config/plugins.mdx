...
- implementation
  - 类型：`string`
  - 作用：传入一个路径，作为 Module Federation 1.5 运行时部分的实现，默认为 [@module-federation/webpack-bundler-runtime](https://github.com/module-federation/universe/tree/main/packages/webpack-bundler-runtime)。
- runtimePlugins
  - 类型：`string[]`
