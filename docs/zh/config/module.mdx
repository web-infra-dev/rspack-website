...
与 `output.assetModuleFilename` 相同，但用于特定规则。覆盖 `output.assetModuleFilename`，只对 asset 和 asset/resource 模块类型起作用。
...
- **类型：** `'javascript/auto' | 'typescript' | 'json' | 'asset' | 'asset/source' | 'asset/resource' | 'asset/inline'`
...
- `'json'`: JSON data 模块
- `'asset' | 'asset/source' | 'asset/resource' | 'asset/inline'`: 参考 Asset Module
  {
  //TODO: Add Link to asset module
  }
...
用于传递 Loader 包名与其选项的数组。`string[]` 如: `use: ['svgr-loader']` 是 `use: [ { loader: 'svgr-loader' } ]` 的简写。
Loaders 会按照从右到左的顺序执行。
