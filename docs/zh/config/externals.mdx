...
将默认的 externals 设置为 `'module'`。Webpack 将为模块中使用的外部对象生成像 `import * as X from '...'` 的代码。

注意，输出 bundle 中将会有 `import` 语句。

### externalsType.node-commonjs

将默认的 externals 类型设置为 `'node-commonjs'`。webpack 将从 `module` 中导入 `createRequire` 来构造一个 require 函数，用于加载模块中使用的外部对象。
