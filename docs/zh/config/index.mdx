...
另外，你也可以使用TypeScript作为配置文件。默认的TypeScript配置文件名称是`rspack.config.ts`。
...
你需要把`ts-node`安装为`devDependencies`，这样rspack才能解析`ts`扩展。
...
注意，如果JS文件不存在，rspack将首先搜索JavaScript，然后才是TypeScript。
