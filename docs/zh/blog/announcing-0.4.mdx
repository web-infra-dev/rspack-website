...
Rsbuild 是基于 Rspack 的构建工具，旨在成为 **更加用户友好和强大的增强型 Rspack CLI**。由于 Rspack 团队对 Web 构建最佳实践的探索和实施，Rsbuild 是那些希望从 Webpack 迁移至 Rspack 的理想解决方案。它能够显著减少 90% 的配置同时提供 5 ～ 10 倍的构建速度。
