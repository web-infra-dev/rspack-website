...
我们很高兴地宣布 **[Rsbuild](https://github.com/web-infra-dev/rsbuild)** **v0.1** 的发布！这个里程碑表明 Rsbuild 已经足够稳定，可用于生产环境。

Rsbuild 是基于 Rspack 的构建工具，旨在成为 **增强版的 Rspack CLI**，更加容易上手且开箱即用。Rsbuild 是 Webpack 应用迁移到 Rspack 的最佳方案，他能帮助你减少 90% 配置并获得 10 倍构建速度。
...
欢迎查看 [Rsbuild 仓库](https://github.com/web-infra-dev/rsbuild) 并开始试用 🙌。
...
`experiments.incrementalRebuild` 已内置开启，所以不再需要改配置。Rspack 将在 v0.5.0 中移除该配置。
...
- Rspack 独有的插件，如 SwcJsMinimizerRspackPlugin 等

如，原有的 `builtins.define` 可以这样迁移：
...
其他内容可以直接参考 rspack [内部插件](/config/plugins.html#%E5%86%85%E9%83%A8%E6%8F%92%E4%BB%B6)，进行迁移。
