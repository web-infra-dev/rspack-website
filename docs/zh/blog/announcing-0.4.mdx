...
新解析器可以配置为读取 `tsconfig.json` 的 `compilerOptions.paths` 和 `references` 字段。有关详细信息，请参阅 API [resolve.tsConfig](https://www.rspack.dev/config/resolve.html#resolvetsconfig)。
...
自 0.4.0 起，Rspack 不再支持 Node.js 14，现在需要 Node.js 16+。

### 使用 `@rspack/core` 手动安装 `@rspack/cli`
...
### 对于 React 应用程序，请使用 `@rspack/plugin-react-refresh`
