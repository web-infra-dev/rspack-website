...
当您将 `mode` 设置为 `production` 时，将启用 tree shaking。但是您还可以通过将 `builtins.treeShaking` 设置为 `true` 来在 **development** 模式下启用它。
...
注意到，我们没有从 **src/math.js** 模块导入 `square` 方法。该函数是“死代码”，表示应该删除未使用的导出。现在让我们构建我们的项目
将产生以下构建结果：
...
正如您所看到的，如果我们不启用 treeShaking，则所有代码都保持不变，只是将所有的代码都报了运行时代码。
...
`random` 变量在默认情况下是需要被保留的，因为它在模块初始化时期包含副作用。
...
正如我们所看到的，**math/index.js**、**math/subtract.js** 和 **math/multiply.js** 都没有副作用，而 math/add.js 不是，因为 `const randomDate = Date.now()` 含有副作用。当我们重新构建项目时，您可以看到差异如下：
...
我们知道，只有在`addRandomDate` 导出被使用时, 初始化的副作用才有意义，而当前项目中该变量并没有被使用，因此我们可以安全地覆盖它，我们修改 rspack.config.js:
...
重新构建项目，我们得到和之前相同的结果，整个 **math/add.js** 模块都被省略了。
