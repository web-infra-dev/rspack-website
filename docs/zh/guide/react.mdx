...
我们意识到这一问题，希望通过 [rspackFuture.disableTransformByDefault]() 和 [@rspack/plugin-react-refresh](/guide/react.html#rspackplugin-react-refresh) 来帮助用户逐渐地迁移到更为正确的方式上来，即第二种方式。
...
:::warning
`@rspack/plugin-react-refresh` 依赖于 [rspackFuture.disableTransformByDefault]()
:::

[React Fast Refresh](https://reactnative.dev/docs/fast-refresh) 功能的开启主要分为两部分：代码注入和代码转换

- 代码注入可通过 [@rspack/plugin-react-refresh](https://github.com/web-infra-dev/rspack/tree/main/packages/rspack-plugin-react-refresh) 开启
- 代码转换可通过 loader 来添加的，比如 `swc-loader` 的 [jsc.transform.react.refresh](https://swc.rs/docs/configuration/compilation#jsctransformreactrefresh) 或 `babel-loader` 的 [react-refresh 插件](https://github.com/facebook/react/tree/main/packages/react-refresh)。
