...
Loader 可以对文件内容进行转换，你可以借助 loader 将 Rspack 中非一等公民支持的文件转换为一等公民类型以获得 Rspack 的支持。

:::tip

Loader 属于对资源的前处理，而 [Rule.type](/config/module#ruletype) 则属于对资源的后处理，它们定义了 Rspack 支持的一等公民类型。
...
CSS 为 Rspack 的一等公民，因此对于 CSS 文件的处理是原生支持的。而对于 `Less` 文件来说则需要借助外部的 [less-loader](https://github.com/webpack-contrib/less-loader) 对文件的内容做相应的转换：
...
你可以对特定的 [Rule](/config/module#rule) 匹配项使用多个 loader 进行链式处理，loader 的执行顺序为从右往左。

例如你可以使用 [less-loader](https://github.com/webpack-contrib/less-loader) 完成 Less 到 CSS 类型之间的转换，并对转换后的源码使用 [postcss-loader](https://github.com/webpack-contrib/postcss-loader) 进行二次转换，并最终交给 Rspack 的 CSS 后处理器完成进一步处理：
