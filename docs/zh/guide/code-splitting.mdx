...
为了更容易地理解输出结果，我们可以使用一些工具来分析 Rspack 输出的结果。

Rspack 内置了类似 [`webpack-bundle-analyzer`](https://github.com/webpack-contrib/webpack-bundle-analyzer) 插件的功能，你可以通过在构建的时候添加 `--analyze` 参数来启用它。

```
pnpm rspack build --analyze
```
