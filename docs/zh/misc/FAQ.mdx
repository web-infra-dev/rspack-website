...
我们和 Webpack 团队确立了合作关系，Rspack 作为 Webpack 通过 Rust 进行性能优化的一个尝试，我们会和 Webpack 团队一起探索如何通过 Rust 进行 Webpack 的性能优化，当 Rspack 实现了 80% 的 Webpack 功能后，Webpack 将尝试以实验特性的方式接入 Rspack。
...
Rspack 内部使用 SWC 进行代码的降级编译（可以通过 [presetEnv](/config/builtins#builtinspresetenv)），因此无需通过 babel-loader 来进行代码的降级编译。
