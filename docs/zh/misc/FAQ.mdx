...
Rspack 内部使用 swc 进行代码的降级编译，因此无需通过 babel-loader 来进行代码的降级编译。
...
可以，目前已经在多个内部项目中落地了生产环境，我们目标运行时是完全对齐 webpack(意味着你很难从产物里判断是 Webpack 还是 Rspack 产物)，但是目前离完整对齐还有一点差异,我们将在下个双月完成 Runtime 的完整对齐。
