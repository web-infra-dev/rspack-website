...
Rspack Loader is designed to reuse the Webpack Loader, we are compatible with most of the Webpack Loader API, the rest of the API is still being implemented, so most of the Webpack Loader can already run in Rspack.

## 'Raw' Loader

By default, resource files are converted to UTF-8 strings and passed to the loader. loaders can receive raw `Buffer` by setting `raw` to `true`. Each loader can pass its processing results as `String` or `Buffer`, and the complier will convert them to and from the loader.
...
The path string of the current module, excluding the query parameter. For example `'/abc/resource.js?query#hash'` in `'/abc/resource.js'`.
