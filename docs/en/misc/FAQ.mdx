...
We have established a partnership with the webpack team and see Rspack as an exploration of optimizing webpack using Rust. When Rspack has implemented 80%+ of the webpack functionality, webpack would like to implement a webpack experiments rspack feature that run rspack from webpack itself/node API
...
Rspack internally uses SWC to perform code downgrade compilation which you can configure by [presetEnv](/config/builtins#builtinspresetenv), so there is no need to use babel-loader to perform code downgrade compilation.
...
## When will Rspack support Vue?

Plan in progress, expected to complete support between April and June.
