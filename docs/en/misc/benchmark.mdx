...
This project is used for comparing development mode and production mode build performance, it is a combination of `atlaskit-editor`, `common-libs`, `common-libs-chunks`, `rome`, `esbuild-three`
...
Some tools' HMR benchmarks use the entire HMR time as the metric time, and since each toolchain may implement each stage of HMR differently,
it's fine to compare in this way. However, since we have basically the same implementation as webpack in stages two through four,
we use a more fine-grained incremental build algorithm to recompile the project, to demonstrate the effectiveness of that algorithm,
we use the rebuild time of each build tool as a metric.
