...
### experiments.rspackFuture.disableApplyOptionsLazily
...
When this feature is not enabled, options can still make valid changes to some options after `compiler.hooks.afterEnvironment` is called.

With this feature turned on, options will no longer make valid changes to options after a `compiler.hooks.afterEnvironment` call. This behavior is consistent with that of Webpack, so this configuration is largely unaffected for users developing applications in Rspack, but should be noted by developers of Rspack plugins or upper-level frameworks.

A common scenario is to [make changes to entries after the Compiler has been created](https://github.com/webpack/webpack-dev-server/blob/540c43852ea33f9cb18820e1cef05d5ddb86cc3e/lib/Server.js#L719-L783):
...
prependEntry(compiler, 'client.js');
```
