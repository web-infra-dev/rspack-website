...
Treat references to `http(s)://...` and `std:...` as external and load them via `async import()` (`externalType: "import"`) when used. (Note that this external type is an async module, which has various effects on the executio).
