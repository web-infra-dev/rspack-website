...
`context` is an absolute path that is used as the base path for relative paths in Rspack configurations such as [entry](/config/entry.html) and [loader](/config/loader.html).
