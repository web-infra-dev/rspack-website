...
2. [target](https://www.rspack.dev/config/target) will not downgrade user-side code
...
With `experiments.rspackFuture.disableTransformByDefault` is enabled by default in v0.4.0, `builtin.react.refresh` has also been deprecated. Now we recommend using `@rspack/plugin-react-refresh` to enable react fast refresh. And to use it, you need install [@rspack/plugin-react-refresh](https://www.npmjs.com/package/@rspack/plugin-react-refresh) and [react-refresh](https://www.npmjs.com/package/react-refresh)
...
### Deprecating experiments.incrementalRebuild options

`experiments.incrementalRebuild` has now internally enabled, so you don't need the options anymore, the options has been deprecated. Rspack will remove the options in v0.5.0.
