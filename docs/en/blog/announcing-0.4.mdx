...
The new resolver can be configured to read `tsconfig.json`'s `compilerOptions.paths` and `references` field. See API [resolve.tsConfig](https://www.rspack.dev/config/resolve.html#resolvetsconfig) for details.

To opt out of the new resolver, set `experiments.rspackFuture.newResolver` to `false`.

## Migration Guide

There is an [migrate example](https://github.com/rspack-contrib/rspack-examples/pull/2) demonstrating how to migrate from Rspack 0.3.14 to Rspack 0.4.0

### Choose `@rspack/cli` or `Rsbuild`?

If your application is a CSR application, we strongly encourage you to use Rsbuild other than configure Rspack yourself since Rsbuild is much easier to use compared with `@rspack/cli`.

### Upgrade Node.js Version

Rspack no longer supports Node.js 14 since 0.4.0,Node.js 16+ is required now.

### Install `@rspack/core` manually with `@rspack/cli`

### Use builtin:swc-loader to support module transformation

Rspack no longer transform file by default since 0.4.0, so you need to use `builtin:swc-loader` to transform file now, more details in [Deprecating default transformation](#deprecating-default-transformation).

### Use `@rspack/plugin-react-refresh` for React application

`builtin.react.refresh` is not working when we disable transform by default, so you need to use `@rspack/plugin-react-refresh` to enable fast-refresh, more detail in [Deprecating builtin.react.refresh](#deprecating-builtinreactrefresh).
