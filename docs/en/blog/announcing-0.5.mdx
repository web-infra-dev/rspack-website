...
Some of the builtins options have been deprecated since v0.4.0, and they are removed in v0.5.0, which includes `builtins.noEmitAssets`, `builtins.presetEnv`, `builtins.decorator`, `builtins.devFriendlySplitChunks`, `builtins.react`, `builtins.pluginImport`, `builtins.emotion`, `builtins.relay`, `builtins.html`, `builtins.copy`, `builtins.minifyOptions`. If you are still using them, checkout [migrating builtin options to builtin plugins](https://www.rspack.dev/en/blog/announcing-0.4#%E8%BF%81%E7%A7%BB-builtin-options-%E5%88%B0-builtin-plugins) to migrate.
...
After you upgrade `@rspack/core` to v0.5.0, if you build failed with error: `Failed to resolve ./src/xxx.tsx`, or `Failed to resolve ./src/xxx.ts`, or `Failed to resolve ./src/xxx.jsx`, you need to add `resolve.extensions = ['...', '.tsx', '.ts', '.jsx']` in your configuration.
