...
Module paths are those that do not start with `. `, `/` paths that do not start with `.` In this case, Rspack will resolve the absolute path of the module according to the module resolution rules. The [node module parsing algorithm](https://nodejs.org/api/modules.html#modules_all_together) has a detailed description of the rules for modules resolution algorithm.
