...
Other module types, such as JavaScript modules, are usually bundled into chunk(s) for final bundle generation. In the case of asset modules, it is almost impossible to be bundled, so they usually exist independently. This is one of the most straightforward reasons why it is called a "asset module".
...
- **`'asset/source'`**: Converts and exports the asset file exclusively as a string.

## Example

### Use ``type: 'asset'` to automatically select a mechanism based on conditions
...
### Replace `url-loader` with ``type: 'asset/inline'`
...
There are times when we want to optimize a specific image, such as compressing the size of the image. We can still use these Loaders.

For example, for all files ending in `.png` use the optimization of image-webpack-loader.
