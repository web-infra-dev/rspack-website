...
The plugin is built into `@rspack/dev-server`, which means that if you use `@rspack/cli` or `@rspack/dev-server`, React Fast Refresh will be enabled by default in dev mode. You can also disable React Fash Refresh with [builtins.react.refresh](config/builtins#reactoptions).
