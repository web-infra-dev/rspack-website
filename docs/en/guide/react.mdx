...
Recognizing this issue, we aim to gradually transition users to a more correct approach through [rspackFuture.disableTransformByDefault]() and [@rspack/plugin-react-refresh](/guide/react.html#rspackplugin-react-refresh), which represents the second method mentioned.
...
:::warning
`@rspack/plugin-react-refresh` depends on [rspackFuture.disableTransformByDefault]()
:::
...
- Code injection can be enabled by [@rspack/plugin-react-refresh](https://github.com/web-infra-dev/rspack/tree/main/packages/rspack-plugin-react-refresh).
- Code transformation can be added through loaders, such as [jsc.transform.react.refresh](https://swc.rs/docs/configuration/compilation#jsctransformreactrefresh) for swc-loader or the [react-refresh plugin](https://github.com/facebook/react/tree/main/packages/react-refresh) for babel-loader.
