...
In order to understand the output more easily, we can use some tools to analyze the results of Rspack output.

Rspack has a builtin plugin similar to [`webpack-bundle-analyzer`](https://github.com/webpack-contrib/webpack-bundle-analyzer), You can enable it by adding the `--analyze` parameter to the build.

```
npm run rspack build --analyze
```
