...
First, we need to update the dependencies, uninstall all the Create React App dependencies, then install the @rspack/cli dependencies and update the corresponding startup scripts. To note，`copy-webpack-plugin` is in 5.x, rather than the latest version。Use `npm install copy-webpack-plugin@5` to install.
