...
Because Create React App (CRA) itself has many built-in capabilities, manually setting up an equivalent configuration using `@rspack/cli` can be challenging. Therefore, we recommend migrating CRA applications to [Modern.js Builder](https://modernjs.dev/builder) to leverage Rspack's capabilities.
